...
| 补丁 | 位置 | 作用 |
|------|------|------|
| 0001 | `fs/sysfs/file.c` + `fs/Makefile` | sysfs 假值表：`antimark_sysfs_seq_show()` 命中路径直接输出假值，不落地任何文件 |
| 0002 | `KernelSU/kernel/supercall/dispatch.c` | KSU supercall 分发：`CMD_ANTIMARK_SET (0x555d0)` 路由到 `antimark_handle_cmd()` 注册掉包表 |
| 0003 | `fs/open.c` | `vfs_open()` 文件掉包：命中假表路径时把 `filp` 换成 shmem 假文件（无挂载、`/proc/mounts` 零痕迹） |
...
```
antimark_patch/
├── 0001-antimark-sysfs.patch          # 通用（6.12 / 6.6 / 6.1 均可用）
├── 0002-antimark-dispatch.patch       # 通用（依赖 KSU + SUSFS，见下）
├── 0003-antimark-vfs-fake.patch       # 仅 6.12 版（vfs_open 多行函数体）
├── 0003-antimark-vfs-fake-6.6-6.1.patch  # 仅 6.6/6.1 版（vfs_open 3 行函数体）
├── fs/antimark.c                      # 内核模块主实现（11364 B）
├── include/linux/antimark.h           # 头文件（CMD/OP 定义 + 接口声明）
└── anykernel.sh                       # 刷机包署名模板（改 ATK12 为你自己的）
```

> ⚠️ **0003 两版不可互换**：6.12 的 `vfs_open` 是 `file->f_path = *path; ret = do_dentry_open(...)` 多行体，6.6/6.1 只有 3 行 `file->f_path = *path; return do_dentry_open(file, d_backing_inode(path->dentry), NULL);`。拿 6.12 版打 6.6/6.1 会被 `-F3` fuzz 插进 `dentry_create` 报 `undeclared identifier 'file'`。

## 前置条件

- 内核版本：**6.12 / 6.6 / 6.1**（GKI OKI）
- 必须集成 **KernelSU / ReSukiSU**（0002 要改 `KernelSU/kernel/supercall/dispatch.c`）
- 必须开启 **SUSFS**（`CONFIG_KSU_SUSFS=y`）

> 🚨 **CONFIG_KSU_SUSFS 必须 = y，否则掉包静默失效**：
> `ksu_handle_susfs_cmd()` 整个函数体包在 `#ifdef CONFIG_KSU_SUSFS` 里，0002 补丁的 `CMD_ANTIMARK_SET` case 也在里面。SUSFS 没开时函数被编译器整体删除，`antimark_ctl` 调用 supercall 会**静默 return 0（不报错）**，但掉包表永远注册不上 → drmid 返回真值。排查时先查：`grep CONFIG_KSU_SUSFS arch/arm64/configs/gki_defconfig`。
...
1. **SUSFS 未开 → 掉包静默失效**（见上，本次实测踩过）
2. **CI push 触发会跳过 `if: inputs.xxx` 步骤**：GitHub Actions 中 push 事件没有 inputs，`if: inputs.susfs_enable` 这类条件为 false 会跳过整步。要写成 `if: ${{ inputs.susfs_enable != 'false' }}`
3. **0003 版本混用**：6.12 版补丁打 6.6/6.1 会编译报错，反之 fuzz 插错位置
4. **antimark.c 与 antimark.h 必须同步更新**：内核表结构 / CMD 号变化会导致用户态 ctl 协议失配
